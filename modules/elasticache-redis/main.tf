...
resource "aws_elasticache_replication_group" "this" {
  ...
  description          = "${title(var.family)} Cluster for ${local.name}"

  node_type               = var.node_type
  num_node_groups         = var.shard_number
  ...
  subnet_group_name  = var.existing_subnet_group != "" ? var.existing_subnet_group : aws_elasticache_subnet_group.this[0].name
  security_group_ids = var.existing_security_group != "" ? [var.existing_security_group] : [aws_security_group.this[0].id]
  ...
  data_tiering_enabled       = strcontains(var.node_type, "r6gd")

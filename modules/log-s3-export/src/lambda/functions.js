...
const getExportStatus = async (taskId, cloudwatchlogs) => {
  try {
    const command = new DescribeExportTasksCommand({ taskId: taskId });
    const res = await cloudwatchlogs.send(command);
...
const calcPrefix = (prefix, name, day) => {
  return `${prefix}/${name}/${day.getUTCFullYear()}/${pad(
    day.getUTCMonth() + 1
  )}/${pad(day.getUTCDate())}/exportedlogs`;
};
